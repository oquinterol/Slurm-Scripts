...
module load conda-pws trim-galore pigz multiqc
...
mkdir -p informes_fastqc
# Argumentos adicionales para FastQC
fastqc_args="--outdir informes_fastqc"

# Ejecutar Trim Galore
trim_galore --paired --quality 30 --length 75 --retain_unpaired --cores 4 --trim-n --fastqc_args "$fastqc_args" "$input_R1" "$input_R2"
...
#Compresión informes
zip -r informe_q.zip informes_fastqc/